...
def retrieve_players():
    load_dotenv()
    sqlalchemy_engine = sqlalchemy.create_engine(os.getenv('DATABASE_URL'))
    connection = sqlalchemy_engine.connect()
...
def get_data_from_postgres(query: str, database: str="postgres"):
    load_dotenv()
...
#     load_dotenv()
#     sqlalchemy_engine = sqlalchemy.create_engine(f'postgresql://{os.getenv("DB_USERNAME")}:{os.getenv("DB_PASSWORD")}@{os.getenv("DB_HOST")}:{os.getenv("DB_PORT")}/postgres')
#     connection = sqlalchemy_engine.connect()
#     query = "CREATE TABLE players (player_id SERIAL PRIMARY KEY, player_name VARCHAR(100) NOT NULL, number_of_games INT NOT NULL, wins INT NOT NULL, losses INT NOT NULL, draws INT NOT NULL, points INT NOT NULL, win_rate FLOAT NOT NULL, points_win_rate FLOAT NOT NULL, points_per_game FLOAT NOT NULL);"
#     connection.execute(sqlalchemy.text(query))
...
def add_new_match_to_db(user_id: uuid.UUID, team_1: str="Team_1", team_2: str="Team_2", team_1_rating: float=0.0, team_2_rating: float=0.0, team_1_players: list[uuid.UUID]=[], team_2_players: list[uuid.UUID]=[], team_1_score: int=0, team_2_score: int=0, location: str="Unknown", referee: str="Unknown", match_datetime: datetime.datetime = datetime.datetime.now(), outcome: str = "Unknown", created_datetime: datetime.datetime = datetime.datetime.now(), updated_datetime: datetime.datetime = datetime.datetime.now(), database_name: str="postgres"):
    ...
    with sqlalchemy_engine.connect() as connection:
        for player1 in team_1_players:
            ...
            connection.execute(player_log_insert)
        for player2 in team_2_players:
            ...
            connection.execute(player_log_insert)
        connection.execute(matches_insert)
        connection.commit()

def add_new_player_with_stats_to_db(created_by: uuid.UUID, player_name: str, number_of_games: int=0, wins: int=0, losses: int=0, draws: int=0, points: int=0, win_rate: float=0.0, points_win_rate: float=0.0, points_per_game: float=0.0, created_datetime=datetime.datetime.now(), udpated_datetime=datetime.datetime.now(), database_name: str="postgres"):
    load_dotenv()
    sqlalchemy_engine = sqlalchemy.create_engine(f'postgresql://{os.getenv("DB_USERNAME")}:{os.getenv("DB_PASSWORD")}@{os.getenv("DB_HOST")}:{os.getenv("DB_PORT")}/{database_name}')
    table = sqlalchemy.Table('players', sqlalchemy.MetaData(), autoload_with=sqlalchemy_engine)
    new_uuid = uuid.uuid4()
    ...
    insert = sqlalchemy.insert(table).values(
        ...
        created_datetime=created_datetime,
        updated_datetime=udpated_datetime
    )
    with sqlalchemy_engine.connect() as connection:
...
def get_player_pool_from_db(number_of_players: int, user_id: uuid.UUID, database_name: str="postgres") -> list[dict]:
    ...
    user_idf = user_id
    print(type(user_idf))
    query = f"SELECT * FROM players WHERE created_by = {user_idf};"
    df = pd.read_sql_query(sel, connection)
    print(df)
    num_players = len(df)
    print(f"There are {num_players} players in the database.")
    # players = df.sample(n=number_of_players)
    player_pool = []
...
def update_match_outcome(match_id: uuid.UUID, outcome: str, database_name: str="postgres"):
    load_dotenv()
    sqlalchemy_engine = sqlalchemy.create_engine(f'postgresql://{os.getenv("DB_USERNAME")}:{os.getenv("DB_PASSWORD")}@{os.getenv("DB_HOST")}:{os.getenv("DB_PORT")}/{database_name}')
    table = sqlalchemy.Table('matches', sqlalchemy.MetaData(), autoload_with=sqlalchemy_engine)
    stmt = table.update().where(table.c.match_id == match_id).values(outcome=outcome)
    with sqlalchemy_engine.connect() as connection:
...
def remove_player_from_db(player_id: uuid.UUID, player_name: str, database_name: str="postgres"):
    ...
    sqlalchemy_engine = sqlalchemy.create_engine(f'postgresql://{os.getenv("DB_USERNAME")}:{os.getenv("DB_PASSWORD")}@{os.getenv("DB_HOST")}:{os.getenv("DB_PORT")}/{database_name}')
    table = sqlalchemy.Table('players', sqlalchemy.MetaData(), autoload_with=sqlalchemy_engine)
    stmt = table.delete().where(table.c.player_id == player_id and table.c.player_name == player_name)
    with sqlalchemy_engine.connect() as connection:
        connection.execute(stmt)
        connection.commit()
...
def check_if_user_exists(email: str, username: str, database_name: str="postgres") -> bool:
    ...
    eresult = connection.execute(sqlalchemy.text(equery))
    uquery = f"SELECT * FROM users WHERE username = '{username}';"
    uresult = connection.execute(sqlalchemy.text(uquery))
    # print(result)
...
def authenticate_user(username_email: str, password: str, database_name: str="postgres") -> list:
    load_dotenv()
    sqlalchemy_engine = sqlalchemy.create_engine(f'postgresql://{os.getenv("DB_USERNAME")}:{os.getenv("DB_PASSWORD")}@{os.getenv("DB_HOST")}:{os.getenv("DB_PORT")}/{database_name}')
    connection = sqlalchemy_engine.connect()
    ...
    presult = connection.execute(sqlalchemy.text(pquery))
    uresult = connection.execute(sqlalchemy.text(uquery))
...
def delete_user_account(user_id: uuid.UUID, database_name: str="postgres"):
    load_dotenv()
    sqlalchemy_engine = sqlalchemy.create_engine(f'postgresql://{os.getenv("DB_USERNAME")}:{os.getenv("DB_PASSWORD")}@{os.getenv("DB_HOST")}:{os.getenv("DB_PORT")}/{database_name}')
    user_table = sqlalchemy.Table('users', sqlalchemy.MetaData(), autoload_with=sqlalchemy_engine)
    players_table = sqlalchemy.Table('players', sqlalchemy.MetaData(), autoload_with=sqlalchemy_engine)
    matches_table = sqlalchemy.Table('matches', sqlalchemy.MetaData(), autoload_with=sqlalchemy_engine)
    player_log_table = sqlalchemy.Table('player_log', sqlalchemy.MetaData(), autoload_with=sqlalchemy_engine)
    stmtu = user_table.delete().where(user_table.c.user_id == user_id)
    ...
    with sqlalchemy_engine.connect() as connection:
        connection.execute(stmtu)
        connection.execute(stmtp)
...
def balance_teams(list_of_player_names: list, username: str):
    ...
    all_players = retrieve_user_players(user_uuid, database_name="testing")
    ...
    # all_players[all_players["points_win_rate"].isna()]["points_win_rate"] = 0
    player_names = ['Jona', 'Ash', 'Jolly', 'Jake', 'Jack', 'Koke', 'James', 'Ethan', 'Kal', 'Jan']
...
def process_match_day():
    ...
    stmt = tab.update().where(tab.c.user_id == user_id).values(outcome="Unknown")
    connection.execute(stmt)
    connection.commit()
    get_data_from_postgres(f"SELECT * FROM matches WHERE user_id = {user_id}")
            ...
            while rc is True:
                if update_result == "no":
                    ...
                    if result == "Team 1" or result == "Team 2" or result == "Draw":
                        update_match_outcome(match_id=init_match_id, outcome=result)
    ...
    match_players = get_player_pool_from_db(number_of_players=10, user_id=user_id)
